...
- (AStatement*)cachedStatementForQuery:(NSString*)query {
    return [cachedStatements objectForKey:query];
}

- (void)setCachedStatement:(AStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
    ...
    AResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (AResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (!statement) {
        statement = [[AStatement alloc] init];
    ...
    rs = [AResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (AResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (AResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[AStatement alloc] init];
