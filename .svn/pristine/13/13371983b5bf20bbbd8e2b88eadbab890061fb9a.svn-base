...
#define kJSAnimatedImagesViewImageSwappingAnimationDuration 1.5f

#define kJSAnimatedImagesViewImageViewsBorderOffset 200
...
- (instancetype)initWithFrame:(CGRect)frame isAnimatioin:(BOOL)isAnimation {
...
- (void)_init {
    ...
    float x = -kJSAnimatedImagesViewImageViewsBorderOffset;
    float y = -kJSAnimatedImagesViewImageViewsBorderOffset;
    if (!_isAnimation) {
        x = 0.;
        y = 0.;
    }
    ...
    for (int i = 0; i < 2; i++) {
        UIImageView *imageView = [[UIImageView alloc] initWithFrame:CGRectMake(x, y, self.bounds.size.width + (kJSAnimatedImagesViewImageViewsBorderOffset * 2), self.bounds.size.height + (kJSAnimatedImagesViewImageViewsBorderOffset * 2))];
...
- (void)bringNextImage
{
    ...
    [UIView animateWithDuration:self.timePerImage + kJSAnimatedImagesViewImageSwappingAnimationDuration + kMovementAndTransitionTimeOffset delay:0.0 options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationCurveEaseIn animations:^{
        ...
        if (_isAnimation) imageViewToShow.transform = CGAffineTransformConcat(scaleTransform, translationTransform);
    } completion:NULL];
    
    imageViewToShow.alpha = 1.0;
    imageViewToHide.alpha = 0.0;
    if (_isAnimation)imageViewToHide.transform = CGAffineTransformIdentity;
}
