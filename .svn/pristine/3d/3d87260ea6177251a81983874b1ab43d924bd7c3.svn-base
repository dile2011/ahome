...
@implementation AGuidePageView0
...
- (void)arrayNodeLocation {
    for (AGuideNodeView *nodeView in self.nodeViews) {
        ...
        if (index == 0) {
            nodeLocationValue = [NSValue valueWithCGPoint:CGPointMake(margin_left_size+nodeSize.width/2., nodeSize.height/2.)];
            
        } else if (index == 1) {
            nodeLocationValue = [NSValue valueWithCGPoint:CGPointMake(self.frame.size.width-margin_left_size-nodeSize.width/2., nodeSize.height/2.)];
            
            
        } else {
            float y = (index-1)*(nodeSize.height+margin_node_size)+nodeSize.height/2.;
