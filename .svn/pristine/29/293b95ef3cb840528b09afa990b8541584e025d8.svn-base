    ...
    if (numberOfArguments > 2) {
        for (int i = 0; i < (numberOfArguments - 2); i++) {
            ...
            if (jbb_areObjCTypesEqual(typeForArg, @encode(char))) {
                ...
                [self getArgument:&arg atIndex:index];
                [invocation setArgument:&arg atIndex:index];
            } else if (jbb_areObjCTypesEqual(typeForArg, @encode(Point))) {
                Point arg;
                [self getArgument:&arg atIndex:index];
                [invocation setArgument:&arg atIndex:index];
            } else if (jbb_areObjCTypesEqual(typeForArg, @encode(Size))) {
                Size arg;
                [self getArgument:&arg atIndex:index];
                [invocation setArgument:&arg atIndex:index];
            } else if (jbb_areObjCTypesEqual(typeForArg, @encode(Rect))) {
                Rect arg;
                [self getArgument:&arg atIndex:index];
                [invocation setArgument:&arg atIndex:index];
