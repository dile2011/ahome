...
- (void)hiddenKeyBoard {
    for (QSection *section in [self.root sections]) {
        for (QEntryElement *entryElement in section.elements) {
            if ([[self.quickDialogTableView cellForElement:entryElement] isKindOfClass:[QEntryTableViewCell class]]) {
                QEntryTableViewCell *entryTableView = (QEntryTableViewCell*)[self.quickDialogTableView cellForElement:entryElement];
                [entryTableView handleActionBarDone:nil];
            }
        }
    }
...
- (void)verifyLoginAction:(NSString*)verifyInfo andSection:(NSString*)sectionKey{
    QSection *section = [self.root sectionWithKey:sectionKey];
    self.root.appearance.sectionFooterColor = [UIColor redColor];
    [section setFooter:verifyInfo];
